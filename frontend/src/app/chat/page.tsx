...
const ChatPage = () => {
  ...
  const handleMessageInput = (e:React.FormEvent)=>{
    ...
    console.log("In frontned 1")
    console.log(" goin to backedn")
    
    socket.current?.on("received_message",(data: { username: string; message: string })=>{
  ...
  const handleJoinRoom = (e: React.FormEvent) => {
    ...
    socket.current?.emit("join_room", { username, roomName },(response:string)=>{
      ...
      <div className="w-full h-full border-2 border-black bg-white shadow-lg rounded-lg p-4">
        <div className=" h-[600px] overflow-y-auto border-b border-gray-800 mb-4 p-2">
          {messages?.map((msg, index) => (
            <div key={index} className="p-2 text-black bg-gray-200 rounded my-1">
              <h1>{msg.username} : {msg.message}</h1>   
            </div>
          ))}
        </div>
        <div className="flex gap-2">
          <form className="w-full" onClick={handleMessageInput}>
